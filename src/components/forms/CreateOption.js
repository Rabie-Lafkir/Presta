...
import { createOption } from "@/actions/optionActions";

const CreateOption = ({ service_id }) => {
  const [pRanges, SetpRanges] = useState(1);
  return (
    <form className="w-full" action={createOption}>
      <input
        type="text"
        name="service_id"
        value={service_id}
      ...
      {pRanges < 1 ? (
        ...
        Array.from({ length: pRanges }).map((level, i) => (
          <div className="w-full grid grid-cols-3 gap-2" key={i}>
